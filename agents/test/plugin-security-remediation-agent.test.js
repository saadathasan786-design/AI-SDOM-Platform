...
import { plan, decide } from "../plugin-security-remediation/plugin-security-remediation-agent.js";
...
test("plan() returns exactly two plain-data steps: an advisors step and a generator step", async () => {
  ...
  assert.deepEqual(result.steps[0].ids, ["wordpress-security"]);
...
test("decide() with critical findings (critical=1) returns continue", async () => {
  const step = { type: "advisors", ids: ["wordpress-security"] };
  const result = { success: true, summary: { info: 1, suggestion: 0, warning: 0, critical: 1 } };
  const decision = await decide(step, result);
  assert.equal(decision.action, "continue");
  assert.match(decision.reason, /critical=1/);
});

test("decide() with high/warning findings (warning=1, critical=0) returns continue", async () => {
  const step = { type: "advisors", ids: ["wordpress-security"] };
...
test("decide() with medium volume (suggestion=3, warning=0, critical=0) returns continue -- the volume-based branch", async () => {
  const step = { type: "advisors", ids: ["wordpress-security"] };
...
test("decide() with suggestion volume just below the threshold (suggestion=2, warning=0, critical=0) returns stop", async () => {
  ...
  assert.match(decision.reason, /No meaningful WordPress security findings/);
});
...
test("decide() after a FAILED advisors step returns fail, never continue or stop", async () => {
  const step = { type: "advisors", ids: ["wordpress-security"] };
  const result = { success: false, advisorsFailed: ["wordpress-security"], summary: { info: 0, suggestion: 0, warning: 0, critical: 0 } };
  const decision = await decide(step, result);
  assert.equal(decision.action, "fail");
  assert.match(decision.reason, /WordPress Security scan step failed/);
});
...
test("decide() never inspects individual findings -- identical decision whether findings array is present or absent", async () => {
  ...
  const withFindings = {
    ...
    findings: [{ id: "unescaped-output", severity: "warning", message: "x" }],
  };
