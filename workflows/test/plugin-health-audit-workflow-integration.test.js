...
test("registration: the Plugin Health Audit Workflow is registered and appears in the real Workflow Catalog", () => {
  assert.equal(isAvailable("plugin-health-audit"), true);
  const catalog = listWorkflowCatalog();
  assert.ok(catalog.some((w) => w.id === "plugin-health-audit"));
});

test("catalog discovery: all three real workflows are simultaneously discoverable", () => {
...
test("discovery: metadata exposes correct category, inputRequirements, capabilities, and requiredAgents", () => {
  const meta = getWorkflowMetadata("plugin-health-audit");
  assert.equal(meta.category, "wordpress-health-check");
...
test("compatibility: checkCompatibility confirms all three required agents (two WordPress, one generic) are registered", () => {
  const result = checkCompatibility("plugin-health-audit");
...
test("successful workflow: secure plugin runs all three agents to completion, stopping cleanly after the final one", async () => {
  const report = await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [file("secure.php", SECURE_CODE)] });
  ...
  assert.deepEqual(report.summary.agentsRun, [
    "plugin-security-remediation",
    "plugin-performance-optimization",
    "architecture-remediation",
  ]);
...
test("integration: real agent chaining -- the complete chain runs correctly for a vulnerable plugin (Workflow -> WordPress Agents + generic Agent -> Advisors -> Generators, dry-run)", async () => {
  const report = await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [file("vuln.php", VULNERABLE_CODE)] });
...
test("mixed plugin: workflow completes successfully with a mix of clean and flagged code", async () => {
  const report = await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [file("mixed.php", MIXED_CODE)] });
  assert.equal(report.success, true);
  assert.equal(report.steps.length, 3);
});
...
test("real delegation: zero real filesystem writes occur anywhere across the entire chain", async () => {
  await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [file("vuln.php", VULNERABLE_CODE)] });
...
test("empty input: an empty sourceFiles array produces a clean, successful workflow", async () => {
  const report = await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [] });
  assert.equal(report.success, true);
  assert.equal(report.steps.length, 3);
});
...
test("report structure: the Workflow Report itself uses the exact same shape as every other real workflow", async () => {
  const report = await runWorkflowWithReport("plugin-health-audit", { sourceFiles: [file("secure.php", SECURE_CODE)] });
  assert.deepEqual(
...
test("never-throws contract: running all three real workflows in parallel with the same input does not interfere", async () => {
  const sourceFiles = [file("clean.js", "export const a = 1;")];

  const [healthCheckReport, auditReport, pluginHealthReport] = await Promise.all([
